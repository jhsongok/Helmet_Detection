...
COLAB_ACCESS_PATH = '/content/gdrive/MyDrive/Helmet_Detection'

YOLO_FORMAT_PATH = current_path + '/custom'
TRAIN_IMAGE_PATH = current_path + '/train'
TEST_IMAGE_PATH = current_path + '/test'
...
with open(YOLO_FORMAT_PATH + '/' + 'classes.names', 'w') as names, \
     open(current_path + '/train/' + '_darknet.labels', 'r') as txt:
    for line in txt:
        names.write(line)  
        class_count += 1
